...
def merge_models(models, dataset, arch, tally, round_index):
    ...
    with torch.no_grad():
        ...
        tallysum = 0
        for name, param in new_model.named_parameters():
            param.data.copy_(torch.mul(weights[round_index][name],round_index+1))
            tallysum = 0
...
def average_weights_masks(models, dataset, arch, data_nums):
    print("IN MASkS MODE\n", flush=True)
    new_model = copy_model(models[0], dataset, arch)
    num_models = len(models)
    num_data_total = sum(data_nums)
    print(f"Averaging weights with num_models {num_models} and num_data_total {num_data_total}", flush=True)
    with torch.no_grad():
        # Getting all the weights and masks from original models
        weights, masks = [], []
        for i in range(num_models):
        ...
        for name, buffer in new_model.named_buffers():
            for i in range(1, num_models):
                weighted_masks = masks[i][name] #weighted_masks = torch.mul(masks[i][name], data_nums[i])
            ...
            avg = torch.div(buffer.data, num_models) #avg = torch.div(buffer.data, num_data_total)
            avg = torch.ceil(avg)
...
def create_model(dataset_name, model_type) -> torch.Module:
    ...
    if dataset_name == "mnist": 
        ...
        from archs.cifar10 import mlp, cnn, resnet
        #print("Got CIFAR10")
...
def ftrain(round,
    ...
    num_batch = len(train_loader)
    model.train()
    metric_names = ['Loss','Accuracy', 'Balanced Accuracy','Precision Micro','Recall Micro','Precision Macro','Recall Macro']
...
def fevaluate(model, data_loader, verbose=True):
    ...
    print('Acc. for classes',classtypes, pclass2, ": ", score['Accuracy'][-1], flush=True)
...
def prune_fixed_amount(model, amount, verbose=True, glob=True):
    ...
    if verbose:
        ...
        print(f'Percent Pruned Globally: {global_prune_percent:.2f}', flush=True)

        
def fprune_fixed_amount_res(model, amount, verbose=True, glob=True):
    parameters_to_prune, num_global_weights, layers_w_count = get_prune_params_res(model)

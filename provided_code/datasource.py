...
def iid_split(num_clients,
    ...
    ###print(user_train_loaders[5], flush=True) ###
...
def non_iid_split(num_clients, num_train_samples_perclass,
    ...
    label_idx_pairs_test = [(i, test_data.targets[i])
                            for i in range(data_size_test)]
    label_idx_pairs_test = sorted(
        label_idx_pairs_test, key=lambda pair: pair[1])
    ...
    # Test data loaders
    test_loaders = []
    print(train_digit_list)
    for i in range(0, num_clients):

        # Changed replace to True. EDITOR
        digits = np.random.choice(range(10), 2, replace=True)
        ...
        cur_train_idx = np.random.choice(
            train_digit_list[digit1], num_train_samples_perclass + num_validation_samples_perclass, replace=False)
        cur_test_idx = np.random.choice(
            test_digit_list[digit1], num_test_samples_perclass, replace=False)

        cur_train_idx = np.append(cur_train_idx, np.random.choice(
            train_digit_list[digit2], num_train_samples_perclass + num_validation_samples_perclass, replace=False))
        cur_train_idx = shuffle(cur_train_idx)

        if(digit1 == digit2):  # EDITOR same class
            pass  # EDITOR
        else:  # EDITOR
            cur_test_idx = np.append(cur_test_idx, np.random.choice(
                test_digit_list[digit2], num_test_samples_perclass, replace=False))
...
def get_data_noniid(num_users, n_class, nsamples, batch_size=32, rate_unbalance=1.0):
...
def DataLoaders(num_users, dataset_name, n_class, nsamples, mode="non-iid", batch_size=32, rate_unbalance=1.0):
    if mode == "non-iid":
        if dataset_name == "mnist":
            return get_data_noniid_m(num_users,
                                     n_class,
                                     nsamples,
                                     batch_size,
                                     rate_unbalance)
        elif dataset_name == "cifar10":
            return get_data_noniid(num_users,
                                   n_class,
                                   nsamples,
                                   batch_size,
                                   rate_unbalance)
    else:
        print("Choose other function XD")
        exit()


def get_data_noniid_m(num_users, n_class, nsamples, batch_size=32, rate_unbalance=1.0):
...
def get_data(num_clients, dataset_name,
             ...
             batch_size=4,
             min_shard=1,
             max_shard=30, num_validation_samples_perclass=0,  # EDITOR changed val sam to 0
             num_test_samples_perclass=1000, rate_unbalance=1.0):  # EDITOR changed samples per class to 1000
    ...
    if mode == "iid":
        ...
        print("Supported data splitting schemes: {}, {}, {}".format(
            '"iid"', '"non-iid"', '"non-iid-unequal"'))
        exit()
...
if __name__ == "__main__":
    ...
    users_data, global_test_loader = get_data(
        num_clients=400, dataset_name="cifar10", n_class=2, num_train_samples_perclass=20, mode="non-iid", batch_size=10)
    ...
    for data, label in train_loaders[0]:
        if (count == 0):
            print(data[0])
            plt.imshow(data[0].permute(1, 2, 0)*stds +
                       means, interpolation='bicubic')
            plt.show()
        print(label)
        count += 1
    print(count)

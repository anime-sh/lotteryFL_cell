...
def non_iid_split(num_clients, num_train_samples_perclass,
    ...
    for i in range(0, num_clients):
        ...
        digits = np.random.choice(range(10), 2, replace = True) ###Changed replace to True. EDITOR
...
def get_data_noniid(num_users, dataset_name, n_class, nsamples, mode="non-iid", batch_size=32, rate_unbalance=1.0):

    train_data, test_data = [], []
    train_data, test_data, user_train, user_test = get_dataset_cifar10_extr_noniid(num_users, n_class, nsamples, rate_unbalance)
    ...
    for i in range(num_users):
        ...
        sampler_train = torch.utils.data.BatchSampler(torch.utils.data.SubsetRandomSampler(user_train_temp), batch_size, drop_last=False)
        loader_train = torch.utils.data.DataLoader(train_data, batch_sampler=sampler_train)
        train_loaders.append(loader_train)
        
        sampler_test = torch.utils.data.BatchSampler(torch.utils.data.SubsetRandomSampler(user_test_temp), batch_size, drop_last=False)
        loader_test = torch.utils.data.DataLoader(test_data, batch_sampler=sampler_test)
        test_loaders.append(loader_test)
    
    return train_loaders, test_loaders, test_loader

def get_data_noniid_m(num_users, dataset_name, n_class, nsamples, mode="non-iid", batch_size=32, rate_unbalance=1.0):

    train_data, test_data = [], []
    train_data, test_data, user_train, user_test = get_dataset_mnist_extr_noniid(num_users, n_class, nsamples, rate_unbalance)
